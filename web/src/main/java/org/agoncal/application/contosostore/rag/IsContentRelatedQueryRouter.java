package org.agoncal.application.contosostore.rag;
...
public class IsContentRelatedQueryRouter implements QueryRouter {
  ...
  private static final SystemMessage SYSTEM_MESSAGE = new SystemMessage("""
    You are the official query router assistant for Contoso Store. Your role is to make sure that the user query is related to the business of the Contoso Store or not. Answer only 'yes' or 'no'.
  ...
  private static final PromptTemplate USER_MESSAGE_TEMPLATE = PromptTemplate.from("""
    Is the following query related to the business of Contoso Store? Answer only 'yes' or 'no'
